...
- [ ] 수집 대상(데이터셋) 테이블 생성 — `on_data.scrap_target_dataset`
...
- [ ] `public_service_reservations` 테이블 DDL 작성 (계획서 RDB 스키마 기준)
- [ ] `collection_history` 테이블 설계 — 수집 일시, API명, 수집 건수, 신규/변경/유지 건수, 성공/실패 상태, 에러 메시지
- [ ] JPA Entity + Repository 작성
- [ ] 인덱스/제약 — `service_id` UNIQUE, `service_status`, `area_name`, `receipt_end_dt` 등 계획서 명시 인덱스 설정

### Phase 4. 서울시 Open API 클라이언트 구현 (T2)

- [ ] 5개 API(OA-2266 ~ OA-2270) 공통 호출 클라이언트 (WebClient 기반)
- [ ] 응답 DTO(24개 필드) 매핑
- [ ] 페이지네이션 — `START_INDEX` / `END_INDEX` 반복 호출 (한 번에 최대 1000건)
- [ ] API 키 외부화 (`application.yml`)
- [ ] 재시도 정책 — 최대 3회, 지수 백오프

### Phase 5. 데이터 변환기 구현 (T3)

- [ ] API 응답 DTO → `PublicServiceReservation` 엔티티 매퍼
- [ ] 날짜 문자열 → `LocalDateTime` 파싱 (서울시 API 포맷 대응)
- [ ] X/Y 좌표 null 체크 및 기본값 처리 (Phase 8 Geocoding fallback 연계)
- [ ] DTLCONT 원문 저장 (HTML 정제는 AI 벡터화 시점에 처리)
